...
const Menu = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      {/* 메뉴 컨테이너 - 중앙 정렬, 최대 430px */}
      <div className="flex h-screen w-full max-w-[430px] flex-col bg-purple-500">
        {/* 메뉴 헤더 */}
        ...
        <div className="flex flex-col">
          {menuItems.map((item) => (
            <div key={item.id} className="flex flex-col  ">
              {/* 텍스트 버튼 (클릭 가능 영역) */}
              <button
                className="relative text-left text-xl font-bold leading-[18px] text-gray-50 hover:opacity-80 py-[28px] mx-6 px-8 transition-opacity before:absolute before:inset-0 before:bg-transparent hover:before:bg-purple-600 "
                onClick={() => {
