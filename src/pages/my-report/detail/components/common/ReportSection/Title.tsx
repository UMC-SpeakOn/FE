...
const Title = ({ title, description }: TitleProps) => {
  return (
    <div className="w-full flex items-center gap-[0.7rem] min-h-[2.3rem]">
      <p className="whitespace-nowrap font-bold text-[clamp(1.8rem,1.5vw,1.9rem)] text-black leading-none">
        {title}
      </p>
      ...
      {description && (
        <div className="w-full relative flex items-center">
          ...
          <div className="absolute py-[0.58rem] px-[0.87rem] bg-purple-50 rounded-[3.431rem] ml-2.5">
            <p className="font-semibold text-[clamp(0.9rem,1vw,1.2rem)] leading-none text-purple-300">
              {description}
            </p>
          </div>
        </div>
